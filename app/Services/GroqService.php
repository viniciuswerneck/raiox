...
class GroqService
{
    ...
    private function getNextKey()
    {
        ...
        if (! $key) {
            ...
            if ($nextToRelease) {
                ...
                Log::warning("Groq: Nenhuma chave livre. Próxima em {$waitSecs}s (ID: {$nextToRelease->id})");
    ...
    private function callGroq(string $prompt, string $model, object $aiKeyRecord): ?string
    {
        ...
        try {
            ...
            if ($status === 429) {
                Log::warning("Groq Key #{$aiKeyRecord->id} atingiu limite (429). Suspendendo por 5 minutos.");
    ...
    /**
     * Verifica se o campo 'historia' tem pelo menos 3 parágrafos (separados por \n\n).
     */
    ...
    /**
     * Se a IA não respeitou os 3 parágrafos, divide o texto em 3 partes iguais forçando a estrutura.
     */
...
PROMPT;
        ...
        foreach ($models as $model) {
            ...
            for ($i = 0; $i < $maxKeyTries; $i++) {
                ...
                // Se já tem 3 parágrafos, perfeito! Retorna agora.
